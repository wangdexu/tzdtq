	...
	Tree.prototype = {
		...
		_init: function(options) {
			...
			if (options.selectedNodeIds) {
				this.selectedNodeIds = $.extend(true, [], options.selectedNodeIds);
				delete options.selectedNodeIds;
			}
		...
		_clickHandler: function(event) {
            ...
            if (classList.indexOf('treeCheckBox') != -1) {
            	 var checked = target.prop('checked');
            	 this._recursiveSelectedSonNode(node,checked);
            	 this._recursiveSelectedParentNode(node.parentId,checked);
            	 this._render();
            	 return;
            }
		...
		getSelectedNodes:function() {
			var returnSelectedNodes = [];
			if (this.options["multSelect"]) {
				$.each(this.nodes, function(i,e) {    
					 if (e.checked) {
					 	returnSelectedNodes.push(e);
					 }
				});
				
			} else {
				if (!$.isEmptyObject(this.selectedNode)) {
				   returnSelectedNodes.push(this.selectedNode);
				}
			}
		...
		_setInitialLevels: function(nodes, level) {
			...
			$.each(nodes, function addNodes(id, node) {
				...
				$.each(self.selectedNodeIds, function(id,selectNodeId) {    
		...
		_recursiveSelectedSonNode: function(parentNode,isSelected) {
			if (!parentNode) {
				return;
			}
			parentNode.checked = isSelected;
		    var sonNodes = parentNode.nodes ? parentNode.nodes : parentNode._nodes ? parentNode._nodes : undefined;
            if (!sonNodes) {
               return;
            }
            var self = this;
			$.each(sonNodes, function checkNodes(id, sonNode) {
				return self._recursiveSelectedSonNode(sonNode, isSelected);
			});
		},
		
		_recursiveSelectedParentNode: function(parentId,isSelected) {
			if (parentId == "") {
				return;
			}
			var currentNode = this._findNodeById(parentId);
			if (!isSelected) {
				 currentNode.checked = false;
				this._recursiveSelectedParentNode(currentNode.parentId,isSelected);
			} else {
				var sonNodeSelected = true;
				$.each(currentNode.nodes, function(i,sonNode) {    
					if (!sonNode.checked) {
						sonNodeSelected = false;
					}
				});
				currentNode.checked = sonNodeSelected;
				this._recursiveSelectedParentNode(currentNode.parentId,isSelected);
			}
		},
		...
		_buildTree: function(nodes, level) {
			...
			$.each(nodes, function addNodes(id, node) {
				...
				if (self.options["multSelect"]) {
					var checkBoxTemplate = $(self._template.checkBox);
				    checkBoxTemplate.prop("checked",node.checked ? true :false);
				    treeItem.append(checkBoxTemplate);
				} else {
					treeItem.append($(self._template.icon));
				}
				// Add text
		...
		_template: {
			...
			icon: '<span class="singleSelected"></span>',
			checkBox:'<input class="treeCheckBox" name="treeCheckBox" type="checkbox" />',
			link: '<a href="#" style="color:inherit;"></a>',
			badge: '<span class="badge"></span>'
		},

		_css: '.treeview .list-group-item{cursor:pointer}.treeview span.indent{margin-left:10px;margin-right:10px}.treeview span.expand-collapse{width:1rem;height:1rem}.treeview span.singleSelected{margin-left:10px;margin-right:5px}.treeview input.treeCheckBox{margin-left:10px;margin-right:5px}'
		// _css: '.list-group-item{cursor:pointer;}.list-group-item:hover{background-color:#f5f5f5;}span.indent{margin-left:10px;margin-right:10px}span.singleSelected{margin-right:5px}'
	  ...
	  this.each(function() {
			...
			if (typeof options === 'string') {
				if (!self) {
					...
					value = self[options].apply(self, args);
				}
			}
			else {
				if (!self) {
